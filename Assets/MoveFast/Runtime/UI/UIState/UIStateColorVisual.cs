...
{
    /// <summary>
    /// Modifies a graphics color in response to ui hover events
    /// </summary>
    [ExecuteAlways]
    public class UIStateColorVisual : UIStateVisual
    {
        ...
        {
#if UNITY_EDITOR
            if (!this || !TryGetComponent(out _renderer)) return;
#endif
            ...
            else
            {
                ...
                TweenRunner.Tween01(_duration, x => _renderer.SetColor(Color.Lerp(current, color, x))).SetID(this);
            }
        }
    }

    ...
    public class Rectangle : MaskableGraphic
    {
        ...
        private float UsedRadius
        {
            get
            {
                ...
                return ClampRadiusToHalfRectSize(usedRadius);
            }
        }
        ...
        private void DrawStroke()
        {
            ...
            if (_antiAlias > 0)
            {
                ...
                var outerAA = AddRectangle(radius, radius + aa * 0.5f, edgeColor);
                var outer = AddRectangle(radius, radius - aa * 0.5f, color);
                var inner = AddRectangle(radius, radius - _stroke + aa * 0.5f, color);
                var innerAA = AddRectangle(radius, radius - _stroke - aa * 0.5f, edgeColor);
            ...
            {
                var outer = AddRectangle(radius, radius, color);
                var inner = AddRectangle(radius, radius - _stroke, color);
                Bridge(outer, inner);
            }
        }
        ...
        {
            var topLeft = Bridge(a.topLeft, b.topLeft);
            var topRight = Bridge(a.topRight, b.topRight);
            var bottomRight = Bridge(a.bottomRight, b.bottomRight);
            var bottomLeft = Bridge(a.bottomLeft, b.bottomLeft);
        ...
        private BridgedCornerSegmentEdges Bridge(CornerSegment a, CornerSegment b)
        {
            if (a.count == b.count)
            {
        ...
        private void DrawRectangle()
        {
            ...
            if (_antiAlias > 0)
            {
                var outerAA = AddRectangle(radius, radius + _antiAlias * 0.5f, new Color(color.r, color.g, color.b, 0));
                var fill = AddRectangle(radius, radius - _antiAlias * 0.5f, color);
                Bridge(outerAA, fill);
                Triangulate(fill);
            }
            else
            {
                var fill = AddRectangle(radius, radius, color);
                Triangulate(fill);
            }
        }
        ...
        private RectangleSegment AddRectangle(Vector4 insets, Vector4 radii, Color32 color, Vector3 offset, bool affectByGradient = true, bool clampOffset = false)
        {
        ...
        {
            if (_shadowUVMode) return new Vector2(0.5f, 0.5f);
        ...
        private void DrawDropShadow(Shadow shadow)
        {
            ...
            if (shadow.blur > 0)
            {
                ...
                var inner = AddRectangle(centerInset, innerRadius, middleColor, offset, false);
                Bridge(outer, inner);
                Triangulate(inner);
            }
            else
            {
                var rectangle = AddRectangle(centerInset, outerRadius, shadow.color, offset, false);
        ...
        private void DrawInnerShadow(Shadow shadow)
        {
            ...
            _shadowUVMode = true;
            var outer = AddRectangle(centerInset, clampedOuterRadius, outerColor, offset, false, true);
            var inner = AddRectangle(centerInset, innerRadius, edgeColor, offset, false, true);
            _shadowUVMode = false;
            ...
            {
                var original = AddRectangle(radius, UsedRadius, shadow.color, Vector3.zero, false);
